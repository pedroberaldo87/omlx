...
def _mtp_compat_for_model(model_info: dict) -> tuple[bool, str]:
    ...
    # Same acceptance as upstream #3200 (qwen4_exp bypasses the mlx-lm
    # whitelist and relies on the embedded mtp.* weight check below), named:
    # _is_mtp_settable is the dashboard gate, _is_mtp_compatible the runtime.
...
async def update_model_settings(
    ...
    if "mtp_enabled" in sent:
        ...
        if new_mtp_enabled:
            ...
            model_type = cfg.get("model_type")
            # Same acceptance as upstream #3200, via the named dashboard gate
            # (see _mtp_compat_for_model); the mtp.* weight check below stays.
            if not _is_mtp_settable(cfg, model_type):
                raise HTTPException(
                    status_code=400,
                    detail=(
