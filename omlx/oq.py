...
# to wq_b/wk/weights_proj — the model authors protect the whole indexer.
# Without them here the two pooling projections only reach Q8 when the
# budget planner happens to have room, instead of by rule.
_GLM_INDEXER_PROJECTIONS = (
    "wq_b",
    "wk",
    "weights_proj",
...
def universal_quant_predicate(
    ...
    # n_routed_experts is the only key GLM-5.x (glm5_next) ships; without it the
    # predicate saw 0 experts, took is_moe=False and treated a 97%-MoE model as
    # dense — which also pinned gs() to 64 instead of 128. _nested_config_int
    # with the same three keys is what line ~3640 of this file already uses.
    ...
    if path_l.endswith("dt_bias"):
        return False
    # GLM-5.x (glm5_next) names these self_attn.{q,k,v}_conv1d and
    # self_attn.o_proj, so the "linear_attn" guard never matched and 102 conv
    # tensors plus 34 output projections fell through to the base bits. The
